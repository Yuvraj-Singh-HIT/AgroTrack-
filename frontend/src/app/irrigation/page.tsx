...
export default function IrrigationPage() {
  ...
  const sensor = useRealtimeSensor(firestore, "zone-1");
  ...
  // Merge live sensor moisture into the first zone when available.
  useEffect(() => {
    ...
    setZones((prev) =>
      prev.map((z) =>
        z.id === 1
          ? {
              ...z,
              moisture: sensor.reading!.moisture,
              temp: Math.round(sensor.reading!.temperature),
            }
          : z
      )
    );
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description={`Real-time monitoring (${sensor.source === "firestore" ? "live sensor" : "simulator"})`}
      />
