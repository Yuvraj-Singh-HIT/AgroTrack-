...
const prompt = ai.definePrompt({
  ...
  output: { schema: SoilAnalysisOutputSchema.omit({ explainability: true }) },
  ...
  async input => {
    ...
    return {
      ...
      explainability: {
        ...
        confidenceBasis: 'Interpretation of farmer-entered lab-style soil parameters (not a computer vision model).',
        evidence: [
          `pH: ${input.ph}`,
          `Nitrogen: ${input.nitrogen} ppm`,
          `Phosphorus: ${input.phosphorus} ppm`,
          `Potassium: ${input.potassium} ppm`,
          `Moisture: ${input.moisture}%`,
          `Organic matter: ${input.organicMatter}%`,
        ],
        reasoning:
          'Gemini compared each parameter against typical agronomic optimal ranges and synthesized fertilizer, amendment, and irrigation advice from those inputs.',
        fallbackUsed: false,
      },
    };
  }
